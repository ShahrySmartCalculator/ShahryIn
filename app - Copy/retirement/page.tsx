        ...
        <div id="report-content" className="bg-white rounded-xl shadow p-6 mt-6 overflow-x-auto space-y-8">
          ...
          {data.length > 0 ? (
            <>
                ...
                <table className="w-full min-w-[600px] border-collapse border text-center text-sm">
                  ...
                  <tfoot className="font-semibold bg-gray-100">
                    <tr>
                      ...
                      <td className="border p-2 font-mono text-blue-900">{totalSalary.toLocaleString('ar-IQ')}</td>
                      <td className="border p-2 font-mono text-blue-900">{totalCut.toLocaleString('ar-IQ')}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
              ...
              <div className="mt-8 bg-white rounded-lg p-4 border border-gray-300 print:border-black print:border-2">
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-6 text-center">
                  {['عضو لجنة', 'عضو لجنة', 'عضو لجنة', 'رئيس اللجنة'].map((label, idx) => (
                    <div key={idx} className="flex flex-col items-center">
                      <label className="font-bold mb-3 text-gray-700">{label}</label>
                      <input
                        type="text"
                        placeholder="اسم"
                        value={committeeNames[idx]}
                        onChange={(e) => handleCommitteeNameChange(idx, e.target.value)}
                        className="border-t-2 border-black w-full max-w-[290px] text-center text-lg font-semibold focus:outline-none focus:border-blue-500 transition"
                      />
                    </div>
                  ))}
                </div>
