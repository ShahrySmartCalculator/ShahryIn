          ...
          <div id="report-content" className="bg-white rounded-xl shadow p-4 print:shadow-none overflow-auto">
              ...
              <table className="min-w-[900px] w-full border-collapse border text-center text-sm">
                ...
                <tbody>
                  {payments.map((p) => {
                    ...
                    // Calculate credits and debits by summing payments_entries dynamically
            ...
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div key={idx} className={`rounded-xl p-4 shadow font-[Cairo] text-center ${bg} ${border} border`}>
                  <div className="text-gray-700 font-semibold text-sm mb-2">{label}</div>
            ...
            {/* Employee Count */}
            <div className="mt-4 rounded-xl p-4 shadow font-[Cairo] bg-indigo-50 border border-indigo-200 flex justify-center items-center gap-3">
              <div className="text-gray-700 font-semibold text-sm">عدد الموظفين في التقرير:</div>
              <div className="text-blue-900 font-extrabold text-xl font-mono">{payments.length}</div>
            </div>

            {/* Committee Signature */}
            <div className="mt-4 w-full bg-white rounded-xl shadow-md p-4 text-sm font-[Cairo]">
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-6 text-center">
                {['عضو', 'عضو', 'عضو', 'رئيس اللجنة'].map((role, idx) => (
                  <div key={idx} className="flex flex-col items-center">
                    <label className="font-bold mb-3 text-gray-700">{role}</label>
                    <input
                      type="text"
                      placeholder="اسم"
