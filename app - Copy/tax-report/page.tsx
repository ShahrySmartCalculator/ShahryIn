...
const toArabicNumber = (num: number) =>
  new Intl.NumberFormat('ar-EG', { useGrouping: true }).format(num);
...
export default function TaxReportPage() {
  ...
  const fetchReportData = async () => {
    ...
    // ✅ فلترة موظفي نفس الدائرة (وليس الفروع)
    const filtered = (data || []).filter(
      (p) => p.employees?.offices?.auth_user_id === user?.id
    );
                  ...
                  {['عضو اللجنة', 'عضو لجنة', 'عضو لجنة', 'رئيس اللجنة'].map((label, idx) => (
                    <div key={idx} className="flex flex-col items-center gap-2">
                      ...
                      <input
                        ...
                        className="border-t-2 border-black w-full max-w-[160px] text-center text-lg font-semibold focus:outline-none print:border-black print:bg-transparent"
                      />
                    </div>
                  ))}
                </div>
