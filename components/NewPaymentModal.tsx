...
export default function NewPaymentModal({ onClose, onSaved, payment }: Props) {
  ...
  // Fix: degree and level can be number or null
  const [degree, setDegree] = useState<number | null>(1);
  ...
  const handleDelete = async () => {
    ...
    if (!window.confirm('هل أنت متأكد أنك تريد حذف هذا الراتب؟')) return;
    try {
    ...
    try {
      ...
      if (entries.length > 0) {
        const formattedEntries = entries.map((e) => ({
          ...
          title: e.title === '__custom__' ? e.customTitle ?? '' : e.title,
          amount: e.amount,
          type: e.type,
        }));
